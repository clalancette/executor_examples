...
class SingleThreadedSingleReentrantCBGroup final : public rclcpp::Node
{
public:
  SingleThreadedSingleReentrantCBGroup()
  : Node("single_threaded_single_reentrant_cb_group")
  {
    ...
    timer_ = this->create_wall_timer(
      std::chrono::milliseconds(500), std::bind(&SingleThreadedSingleReentrantCBGroup::timer_callback, this), cb_grp_);
  }
...
int main(int argc, char * argv[])
{
  ...
  fprintf(stderr, " reentrant callback group.  The executor is single-threaded.  The timer attempts to\n");
  ...
  auto service_caller = std::make_shared<SingleThreadedSingleReentrantCBGroup>();
