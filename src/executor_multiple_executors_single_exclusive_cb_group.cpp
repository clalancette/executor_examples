...
class MultipleExecutorsSingleExclusiveCBGroup final : public rclcpp::Node
{
public:
  MultipleExecutorsSingleExclusiveCBGroup()
  : Node("multiple_executors_single_exclusive_cb_group")
  {
    // The second argument tells create_callback_group to not automatically add the callback group
    // to an executor.
    cb_grp_ = this->create_callback_group(rclcpp::callback_group::CallbackGroupType::MutuallyExclusive, false);
    client_ = this->create_client<example_interfaces::srv::AddTwoInts>("add_two_ints", rmw_qos_profile_services_default, cb_grp_);
    timer_ = this->create_wall_timer(
      std::chrono::milliseconds(500), std::bind(&MultipleExecutorsSingleExclusiveCBGroup::timer_callback, this), cb_grp_);
...
int main(int argc, char * argv[])
{
  ...
  fprintf(stderr, " exclusive callback group.  There are two executors; one in the main thread,\n");
  ...
  auto service_caller = std::make_shared<MultipleExecutorsSingleExclusiveCBGroup>();
