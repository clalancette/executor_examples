...
class MultipleExecutorsMultipleReentrantCBGroup final : public rclcpp::Node
{
public:
  MultipleExecutorsMultipleReentrantCBGroup()
  : Node("multiple_executors_multiple_exclusve_cb_group")
  {
    ...
    // to an executor.
    cb_grp_client_ = this->create_callback_group(rclcpp::callback_group::CallbackGroupType::Reentrant, false);
    ...
    timer_ = this->create_wall_timer(
      std::chrono::milliseconds(500), std::bind(&MultipleExecutorsMultipleReentrantCBGroup::timer_callback, this), cb_grp_timer_);
...
int main(int argc, char * argv[])
{
  ...
  auto service_caller = std::make_shared<MultipleExecutorsMultipleReentrantCBGroup>();
